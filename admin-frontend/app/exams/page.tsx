...
type Question = {
  ...
  correctAnswerText?: string;      // Short / Essay
  marks?: number;
  explanation?: string;
  order?: number;
};
...
function hardReload() {
  ...
  if (anyDoc.startViewTransition) {
    anyDoc.startViewTransition(() => window.location.reload());
  } else {
    window.location.reload();
  }
}

/** Reload the parent page right AFTER the print dialog closes (works for iframes too). */
function scheduleRefreshOnPrintClose(targetWin?: Window | null) {
  let done = false;
  const finish = () => {
    if (done) return;
    done = true;
    // slight delay so browsers finish unfreezing layout/focus
  ...
  // Also listen on the printing window (iframe)
  if (targetWin) {
    try {
      targetWin.addEventListener("afterprint", finish, { once: true });
      targetWin.addEventListener("focus", finish, { once: true });
      targetWin.document.addEventListener("visibilitychange", () => {
        if (targetWin.document.visibilityState === "visible") finish();
      }, { once: true });
      try {
        const mql2 = targetWin.matchMedia("print");
        const onChange2 = (e: MediaQueryListEvent) => { if (!e.matches) finish(); };
        // @ts-ignore
        (mql2.addEventListener ? mql2.addEventListener("change", onChange2) : mql2.addListener(onChange2));
        setTimeout(() => {
          if (done) {
            // @ts-ignore
            mql2.removeEventListener ? mql2.removeEventListener("change", onChange2) : mql2.removeListener(onChange2);
          }
        }, 10000);
      } catch {}
    } catch {}
  ...
  // Safety net: if none of the events fire (older browsers), refresh after 2s.
  setTimeout(() => { if (!done) finish(); }, 2000);
}

/** ANSWERS PDF — shows correct answers only */
function buildAnswerHTML(exam: Exam) {
  ...
  const rows = (exam.questions || []).map((q, i) => {
    ...
    const ans = (q.correctAnswerText ?? "").trim() || "—";
    return `
      <div class="q">
        <div class="q-head">
          <div class="q-title"><span class="q-num">${i + 1}.</span> ${safe(q.text)}</div>
        ...
        <div class="answer"><strong>${label}:</strong> ${safe(ans)}</div>
      </div>
    `;
  }).join("");
...
<style>
  ...
  * { box-sizing:border-box; color:inherit; background:transparent; }
  ...
  @media print { body { padding:0; } .q { break-inside:avoid; page-break-inside:avoid; } }
</style>
</head>
<body>
  <div class="doc">
...
export default function ExamsPage() {
  ...
  const onDownload = async (id: string) => {
    ...
    try {
      ...
      // ⤵ install “refresh on print close” before triggering print fallback
      scheduleRefreshOnPrintClose(iframe.contentWindow || undefined);
      ...
      try {
        ...
        // For the save() path (no print dialog), still refresh quickly:
        setTimeout(hardReload, 30);
      } catch (err) {
        const w = iframe.contentWindow;
        if (!w) throw err;
        // This opens the OS print dialog; our listeners will refresh when it closes
        w.focus();
        w.print();
      }
    } catch (e: any) {
  ...
  const onAnswerDownload = async (id: string) => {
    ...
    try {
      ...
      try {
        ...
        if (!w) throw err;
        w.focus();
        w.print(); // dialog opened; listeners will reload when it closes
      }
    } catch (e: any) {
      alert(e.message || "PDF export failed");
    } finally {
